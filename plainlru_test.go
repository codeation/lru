...
func TestPlainHit(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.title, func(t *testing.T) {
			funcKey := ""
			cache := New(4, func(key string) (int, error) {
				funcKey = key
				return 42, nil
			})
...
func TestPlainOrder(t *testing.T) {
	for _, tt := range []struct {
		title    string
		keys     string
		expected string
		...
		t.Run(tt.title, func(t *testing.T) {
			cache := New(4, func(r rune) (int, error) { return 42, nil })
			...
			for e := cache.queue.Back(); e != nil; e = e.Prev() {
				output += string(e.Value.(*pair[rune, int]).key)
			}
